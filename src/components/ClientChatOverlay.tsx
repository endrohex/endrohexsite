...
interface Message {
  id: string;
  sender: "You" | "Endrohex Team" | "System";
  text: string;
  time: string;
}
...
export const ClientChatOverlay: React.FC<ClientChatOverlayProps> = ({ isOpen, onClose }) => {
  const [messages, setMessages] = useState<Message[]>([
    ...
    {
      id: "initial-2",
      sender: "Endrohex Team",
      text: "Let us know what kind of project you are looking to build. Our team is ready to begin.",
      time: "Just now",
    },
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      let reply =
        "Thank you for reaching out! The Endrohex team has received your live ping. We are currently actively checking details. Drop us a DM on Instagram @endrohex to sync instantly!";
          ...
          >
            ...
            <header className="flex justify-between items-center px-6 py-4 border-b border-white/[0.08] bg-white/[0.01]">
              <div className="flex items-center gap-3">
                <div className="flex -space-x-2">
                  <div
                    className="w-8 h-8 rounded-full border border-white/10 bg-[var(--colors-primary)]/10 flex items-center justify-center text-xs font-semibold text-white shadow-md font-display"
                    title="Aneek Biswas"
                  >
                    A
                  </div>
                  <div
                    className="w-8 h-8 rounded-full border border-white/10 bg-[#00F2FE]/10 flex items-center justify-center text-xs font-semibold text-white shadow-md font-display"
                    title="Spandan Upmanyu"
                  >
                    S
                  </div>
                  <div
                    className="w-8 h-8 rounded-full border border-white/10 bg-[#F35588]/10 flex items-center justify-center text-xs font-semibold text-white shadow-md font-display"
                    title="Devie"
                  >
                    D
                  </div>
                  <div
                    className="w-8 h-8 rounded-full border border-white/10 bg-[#F5A623]/10 flex items-center justify-center text-xs font-semibold text-white shadow-md font-display"
                    title="Pranjal Halder"
                  >
                    P
                  </div>
                </div>
                <div>
                  ...
                  <p className="text-[10px] text-white/50 font-light">Direct Agency Connection (Active)</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="p-4 border-t border-white/[0.08] bg-white/[0.01] flex items-center gap-3">
              <input
                ...
                placeholder="Type a message to the Endrohex team..."
